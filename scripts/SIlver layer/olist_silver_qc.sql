/*
=======================================================================================================
 Data Quality check before loading the data 
=======================================================================================================

Purpose:
	1. We quqlity check every table and columns before and after cleaning.
...
-------------------------------------------------
--Data Qulity check on silver.olist_customers
-------------------------------------------------
...
from silver.olist_customers) t1
where cn>1;
...
select count(*) from silver.olist_orders; --99441 rows

select count(*)
from silver.olist_orders o left join silver.olist_customers c using(customer_id)--99441 rows
...
select distinct upper(trim(customer_state)), customer_state
from silver.olist_customers

select distinct initcap(trim(customer_city)), customer_city
from silver.olist_customers

--checking data consistency and format
select customer_zip_code_prefix from silver.olist_customers
...
--=======================================================================================================
-------------------------------------------------
--Data quality check  on silver.olist_geolocation
-------------------------------------------------
...
select *
from (select *, count(*) over(partition by geolocation_zip_code_prefix) cn
		from silver.olist_geolocation) t1
where cn>1
...
select count(*) from silver.olist_customers--99441

select * from silver.olist_sellers--3095

select * from silver.olist_customers c left join silver.olist_geolocation l on c.customer_zip_code_prefix = l.geolocation_zip_code_prefix --15083733
...
select * --min(geolocation_lat), max(geolocation_lat), min(geolocation_lng), max(geolocation_lng) 
from silver.olist_geolocation
...
select distinct geolocation_city, unaccent(geolocation_city), initcap(
			trim(
				regexp_replace(
...
CREATE EXTENSION IF NOT EXISTS unaccent; ---****** SO IMPORTANT******
...
-------------------------------------------------
--Data quality check  on silver.olist_order_items
-------------------------------------------------

select * from silver.olist_order_items;--112650

--duplicate and null check on key columns
select *
from (select *, count(*) over(partition by order_id) cn
from silver.olist_order_items) t1
where cn>1
...
from silver.olist_products) t1
where cn>1

select * --no duplicates
from (select *, count(*) over(partition by seller_id) cn
from silver.olist_sellers) t1
where cn>1

select count(*) from silver.olist_orders o--99441

select count(*)--112650
from silver.olist_orders o join silver.olist_order_items i using(order_id)

select order_status, count(*) no_items_orders --some order are not present in the order_items
from silver.olist_orders o left join silver.olist_order_items i using(order_id)
where i.order_id is null
group by order_status
order by no_items_orders desc

select count(*)--775 with no items
from silver.olist_orders o left join silver.olist_order_items i using(order_id)
where i.order_id is null

--to check the items that are shipped to confirm the one order that were shipped but no items for it
SELECT * FROM silver.olist_orders WHERE order_id = (
    SELECT o.order_id FROM silver.olist_orders o
    LEFT JOIN silver.olist_order_items oi ON o.order_id = oi.order_id
    WHERE oi.order_id IS NULL AND o.order_status = 'shipped'
);
...
---check the freight_value = 0
select o.order_id, payment_value, total_order_value
from silver.olist_orders o left join (select order_id, sum(price) total_order_value
										from silver.olist_order_items
										group by order_id) i on o.order_id=i.order_id
							left join (select order_id, sum(payment_value) payment_value from silver.olist_order_payments group by order_id) p on o.order_id = p.order_id

where o.order_id in (select order_id
from silver.olist_order_items
where freight_value<=0) and payment_value!=total_order_value

--payment_value doesn't match for this order: "80850824446cd379f755548b967c61b9" 184.88	166.00 - because freight cost allocated to different item in the same order

--seller who offered freight_value =0 and count times
...
SELECT o.order_id, oi.price, oi.freight_value, op.payment_value, op.payment_type
FROM silver.olist_orders o
JOIN silver.olist_order_items oi ON o.order_id = oi.order_id
JOIN silver.olist_order_payments op ON o.order_id = op.order_id
WHERE o.order_id = '80850824446cd379f755548b967c61b9';
...
-------------------------------------------------
--Data quality check  on silver.olist_orders
-------------------------------------------------
...
where   order_approved_at>order_delivered_carrier_date or--1359 rows
		order_approved_at>order_delivered_customer_date  --61 rows

select *
from silver.olist_orders
where order_delivered_carrier_date>order_delivered_customer_date --32 rows --with = its 23 rows

select *
from silver.olist_orders
where order_estimated_delivery_date<=order_delivered_carrier_date ---delivery 
...
-------------------------------------------------
--Data quality check  on silver.olist_order_payments
-------------------------------------------------

select * from bronze.olist_order_payments--103886

select * from silver.olist_order_payments--103886


--duplicate and null check in keys columns
select count(distinct order_id) from silver.olist_order_payments--distinct order_id=99440

select *
from (select * , count(order_id) over(partition by order_id) cn
		from silver.olist_order_payments) t1
where cn>1
...
--payment_installments<=0 or --****getting installments=0***--order ids:
...
"1a57108394169c0b47d8f876acc9ba2d"
select distinct payment_type,initcap(
				trim(
...
----to find which order is not in the orders table. ---order id:"bfbd0f9bdef84302105ad712db648a6c", customer_id: "86dc2ffce2dfff336de2f386a786e574"
select o.*,op.*
from silver.olist_orders o left join silver.olist_order_payments op using(order_id)
where o.order_id not in (select order_id from silver.olist_order_payments) 
--=======================================================================================================

-------------------------------------------------
--Data quality check  on silver.olist_order_reviews
-------------------------------------------------

select * from silver.olist_order_reviews;--99224 rows

--duplicate check on key columns
select *
from (select *, count(*) over(partition by review_id,order_id) cn
from silver.olist_order_reviews) t1 
where cn>1
...
-------------------------------------------------
--Data quality check  on silver.olist_products
-------------------------------------------------

select * from silver.olist_products;--32951
...
select count(*)
from silver.olist_order_items i left join silver.olist_products p using(product_id)
...
from silver.olist_products p left join silver.olist_product_translation t using(product_category_name)
where product_weight_g<=0

select *
from silver.olist_product_translation
...
-------------------------------------------------
--Data quality check  on silver.olist_sellers
-------------------------------------------------

select * from silver.olist_sellers;--3095 - rows total
...
select count(*)--112650
from silver.olist_order_items i left join silver.olist_sellers s using(seller_id)
...
select distinct seller_city, initcap(trim(case when seller_city in ('sp / sp', 'sao paluo', 'sao paulo sp','sao paulop','sao pauo','sp') then 'sao paulo'
								   ...
								   else regexp_replace(unaccent(regexp_replace(seller_city,'[''\´]',' ','g')),'[-/\,].*$','') end)) , seller_state --636
			   
from silver.olist_sellers
order by seller_city
where seller_city ~ '[.-@\/#!$&*()]' or seller_city like '%Sao Bernardo do Campo%'
...
---------------------------------------------------
--Data quality check on silver.olist_product_translation
---------------------------------------------------
select * from silver.olist_product_translation;--71 -total rows
...
--fan out risk check
select count(*) from silver.olist_products;--32951

select count(*) from silver.olist_products p left join silver.olist_product_translation t using(product_category_name)--32951
...
----------------------------------------------------------
--data quality check  on silver.olist_payments_agg
----------------------------------------------------------
...
where order_id in 
(select order_id from silver.olist_payments_agg)--99440
...
----------------------------------------------------------
--data quality check  on silver.olist_order_items_agg
----------------------------------------------------------
